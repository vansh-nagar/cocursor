...
const Navbar = () => {
  ...
  return (
    <motion.nav
      initial={{ y: -100 }}
      animate={{ y: 0 }}
      className="fixed w-full top-0 left-0 right-0 z-50 px-4 sm:px-6 md:px-8 lg:px-12 pt-4 sm:pt-6"
    >
      <div 
        className={`max-w-screen-2xl mx-auto flex justify-between items-center py-4 px-6 rounded-3xl border transition-all duration-300 ${
          isScrolled 
      ...
      >
        <Link href="/" className="flex items-center gap-2 group">
          ...
          <span className="text-white font-bold text-lg tracking-tight hidden sm:block">Cocursor</span>
        </Link>
        ...
        <div className="flex items-center gap-4">
          ...
          <SignedIn>
            ...
            <SignOutButton>
              <button className="text-xs text-zinc-500 hover:text-white ml-2 transition-colors cursor-pointer rounded-none border border-white/5 px-4 py-2">
                Logout
              </button>
            </SignOutButton>
          </SignedIn>
