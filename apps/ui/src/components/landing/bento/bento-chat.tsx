"use client";

import { useEffect, useState } from "react";
import { motion } from "motion/react";
import OrangeButton, { WhiteButton } from "../button/orange-button";
import BentoTextSection from "../bento-text-section";

const useTypewriter = (
  text: string,
  delay: number,
  speed: number = 30,
  resetKey: number = 0,
) => {
  const [displayedText, setDisplayedText] = useState("");
  const [isComplete, setIsComplete] = useState(false);

  useEffect(() => {
    setDisplayedText("");
    setIsComplete(false);

    const timeout = setTimeout(() => {
      let index = 0;
      const interval = setInterval(() => {
        if (index < text.length) {
          setDisplayedText(text.slice(0, index + 1));
          index++;
        } else {
          setIsComplete(true);
          clearInterval(interval);
        }
      }, speed);

      return () => clearInterval(interval);
    }, delay);

    return () => clearTimeout(timeout);
  }, [text, delay, speed, resetKey]);

  return { displayedText, isComplete };
};

const ChatAnimation = () => {
  const [key, setKey] = useState(0);

  const userMessage =
    "Create a responsive pricing card component with 3 plans.";
  const aiMessage = "Started working on the pricing card component.";

  const { displayedText: userText, isComplete: userComplete } = useTypewriter(
    userMessage,
    500,
    25,
    key,
  );
  const { displayedText: aiText, isComplete: aiComplete } = useTypewriter(
    aiMessage,
    2000,
    20,
    key,
  );

  useEffect(() => {
    const interval = setInterval(() => {
      setKey((prev) => prev + 1);
    }, 6000);
    return () => clearInterval(interval);
  }, []);

  return (
    <div key={key} className="">
      <motion.div
        className="flex gap-2 sm:gap-3 z-10 relative"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5, ease: "easeOut" }}
      >
        <img
          className="h-8 w-8 sm:h-10 sm:w-10 flex-shrink-0"
          src="/image/pfp.png"
          alt=""
        />
        <div className="flex gap-2 flex-col flex-1 min-w-0">
          <motion.div
            className="flex justify-between items-center gap-2"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.3, delay: 0.2 }}
          >
            <div className="flex gap-2 sm:gap-3 items-center min-w-0">
              <span className="text-sm sm:text-base truncate">Vansh Nagar</span>
            </div>
            <span className="text-xs flex-shrink-0">4:33</span>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.4, delay: 0.4, ease: "easeOut" }}
          >
            <WhiteButton className="text-left px-2 py-2 bg-white text-xs sm:text-xs relative">
              <span className="invisible">{userMessage}</span>
              <span className="absolute left-2 right-2 top-2">
                {userText}
                {!userComplete && <span className="animate-pulse">|</span>}
              </span>
            </WhiteButton>
          </motion.div>
        </div>
      </motion.div>

      <motion.div
        className="flex gap-2 sm:gap-3 mt-3 sm:mt-4 z-50"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5, delay: 1.8, ease: "easeOut" }}
      >
        <img
          className="w-8 h-8 sm:w-10 sm:h-10 flex-shrink-0"
          src="/image/cursor.png"
          alt=""
        />
        <div className="flex gap-2 flex-col flex-1 min-w-0">
          <motion.div
            className="flex justify-between items-center gap-2"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.3, delay: 1.9 }}
          >
            <div className="flex gap-2 sm:gap-3 items-center min-w-0">
              <span className="text-sm sm:text-base truncate">Cocursor</span>
            </div>
            <span className="text-xs flex-shrink-0">4:35</span>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.4, delay: 2.0, ease: "easeOut" }}
          >
            <OrangeButton className="text-left px-2 py-2 text-xs sm:text-xs relative">
              <span className="invisible">{aiMessage}</span>
              <span className="absolute left-2 right-2 top-2">
                {aiText}
                {!aiComplete && <span className="animate-pulse">|</span>}
              </span>
            </OrangeButton>
          </motion.div>
        </div>
      </motion.div>
    </div>
  );
};

const BentoChat = () => {
  return (
    <div
      style={{
        boxShadow: "inset 0px 11px 31.8px 1px rgba(250, 96, 0, 0.25)",
      }}
      className="rounded-3xl sm:rounded-[40px] md:rounded-[48px] lg:rounded-[56px] border relative overflow-hidden p-4 sm:p-6 md:p-8 flex flex-col justify-between "
    >
      <div
        className="-z-20"
        style={{
          position: "absolute",
          width: "173px",
          height: "173px",
          right: "0",
          top: "-15px",
          background: "#E75900",
          filter: "blur(107.3px)",
        }}
      />
      <ChatAnimation />

      <BentoTextSection
        title="Talk Through Problems, Ship Solutions"
        description="Ask questions, debug issues, and build faster all from a simple conversation."
      />
    </div>
  );
};

export default BentoChat;
